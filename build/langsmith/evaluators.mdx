...
A feedback filter and this setting have separate roles: the filter determines which runs trigger the evaluator, and **Include extended stats** makes the additional fields available to it. To configure a code evaluator that triggers when a feedback key exists, see [Apply a filter to runs that trigger the evaluator](/langsmith/online-evaluations-code#configure-online-evaluators).
