...
The warning is skipped entirely for slash commands, prompts sent programmatically (for example, through [ACP](/oss/python/deepagents/acp) from an editor), contexts below the provider's minimum cacheable size, custom endpoints that are not [trusted](#trust-a-gateway-endpoint-for-cache-policies), and models without pricing information. When the warning cannot be displayed, the message is not sent and the draft is restored instead.
